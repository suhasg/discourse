...
def create_group(name)
  log "  - #{name}..."
  result = Group.exec_sql("INSERT INTO groups (name, created_at) VALUES (:name, now()) RETURNING id", name: name)
  # return new id
...
def create_users
  ...
  @users.values.in_groups_of(10_000, false) do |users|
    ...
    sql = "INSERT INTO users (username, username_lower, name, email, website, title, primary_group_id, trust_level, email_digests, external_links_in_new_tab, bio_raw, created_at, active)
           VALUES #{values.join(",")}
           RETURNING id"
...
def sql_fragment_for_user(user)
  User.sql_fragment("(:username, :username_lower, :name, :email, :website, :title, :primary_group_id, :trust_level, :email_digests, :external_links_in_new_tab, :bio_raw, now(), :active)",
...
def create_categories_groups
  ...
  @categories_groups.each do |category_group|
    ...
    next unless @categories_mapping.size > 0 && @mapped_categories.include?(category_group.category_id)

    create_category_group(category_group)
  end
end

def create_category_group(category_group)
  group = @groups[category_group.group_id]
  category = Category.find(@categories[category_group.category_id].id)
  permissions = CategoryGroup.permission_types[category_group.permissions]
  log "  - #{group.name} have #{permissions} access to #{category.name} category"
  ...
  read_restricted, _ = Category.resolve_permissions([category_group.permissions])

  Category.exec_sql("UPDATE categories SET read_restricted = :read_restricted WHERE id = :id",
                     read_restricted: read_restricted,
                     id: @categories[category_group.category_id].id)
end
...
def update_last_posted_at
  log "Update last_posted_at..."

  sql = <<-SQL
    UPDATE users
    SET last_posted_at = last_posted_ats.max
    FROM (
      SELECT users.id AS user_id, MAX(posts.created_at) AS max
      FROM users
      INNER JOIN posts ON posts.user_id = users.id
      GROUP BY users.id
    ) last_posted_ats
    WHERE id = last_posted_ats.user_id
  SQL
...
def create_user_stats
  log "Creating user stats..."

  sql = <<-SQL
    INSERT INTO user_stats (new_since, user_id)
...
def update_groups_stats
  ...
  sql = <<-SQL
    UPDATE groups g SET user_count = (SELECT COUNT(*) FROM group_users WHERE group_id = g.id)
  SQL
...
def update_posts_stats
  ...
  sql = <<-SQL
    UPDATE posts p
    SET    reply_to_user_id = (SELECT u.id FROM users u JOIN posts p2 ON p2.user_id = u.id AND p2.post_number = p.reply_to_post_number AND p2.topic_id = p.topic_id),
           reply_count      = (SELECT COUNT(*) FROM posts p2 WHERE p2.reply_to_post_number = p.post_number)
  SQL
...
def update_topics_stats
  ...
  sql = <<-SQL
    ...
    SET    last_post_user_id     = (SELECT user_id                  FROM posts WHERE deleted_at IS NULL AND topic_id = t.id ORDER BY created_at DESC LIMIT 1),
           ...
           reply_count           = (SELECT SUM(reply_count)         FROM posts WHERE deleted_at IS NULL AND topic_id = t.id),
           incoming_link_count   = (SELECT SUM(incoming_link_count) FROM posts WHERE deleted_at IS NULL AND topic_id = t.id),
  ...
  # TODO:
  # excerpt --> PrettyText.excerpt(cooked, SiteSettings.post_excerpt_maxlength, strip_links: true)
end

def update_topic_users_stats
  log "Updating topic users stats..."

  sql = <<-SQL
    TRUNCATE TABLE topic_users;

    INSERT INTO topic_users (user_id, topic_id, first_visited_at, last_visited_at, posted, notification_level)
    SELECT   user_id, topic_id, MIN(created_at), MAX(created_at), 't', #{TopicUser.notification_levels[:regular]}
    FROM     posts
    GROUP BY user_id, topic_id;

    UPDATE topic_users
    SET    notification_level = #{TopicUser.notification_levels[:watching]}
    FROM   topic_users tu
    INNER JOIN posts p ON p.topic_id = tu.topic_id AND p.user_id = tu.user_id AND p.post_number = 1
    WHERE  topic_users.topic_id = tu.topic_id AND topic_users.user_id = tu.user_id;
  SQL
